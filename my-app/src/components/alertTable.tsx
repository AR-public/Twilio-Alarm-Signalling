...
const AlarmSignallingApp: React.FC = () => {
  ...
  // Activate alertees based on time elapsed
  useEffect(() => {
    if (isAlarmTriggered) {
      setAlertees((prev) =>
        prev.map((alertee) => ({
          ...alertee,
          isActive: timeElapsed >= alertee.activationDelay,
        }))
      );
    }
  }, [timeElapsed, isAlarmTriggered]);
        ...
        try {
          ...
          setAlertees((prev) =>
            prev.map((alertee) => ({
              ...alertee,
              response:
                data[alertee.phone] === 'YES' ? 'YES' : alertee.response,
            }))
          );
        } catch (error) {
  ...
  const handleTriggerAlarm = async () => {
    ...
    try {
      await fetch('http://localhost:3001/api/trigger', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ alertees }),
      });
    } catch (error) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-50 to-red-50 flex items-center justify-center p-8">
      <div className="max-w-2xl w-full">
        <h1 className="text-5xl font-bold text-center mb-12 text-gray-800">
          Zunairah's Alarm Signalling App
        </h1>
...
export default AlarmSignallingApp;
